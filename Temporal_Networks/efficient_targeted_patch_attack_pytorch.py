...
def test_perturbation(input_oflow_stack, img_arr, img_index, x_loc, y_loc, pert_color, pert_width, max_x, max_y, model):
    ...
    _,_,tsmax = classify_stack(model, tmp_of)
...
BOX_WIDTH = 5  # width of perturbation patch placed on image
BOX_STRIDE = 5  # how much to move a perturbation at any given step
...
for stack, label, seq in test_dataset.read(batch_size=1,shuffle=False):
    ...
    # Generate a random target label s.t. target_label != gt_label
    target_label = random.randint(0,NUM_CLASSES-1)
    while target_label == gt_label:
         target_label = random.randint(0,NUM_CLASSES-1)
    assert(gt_label != target_label)
    ...
    pred,_,smx = classify_stack(test_model, stack)
    gtcnf = smx[0][gt_label]
    targetcnf = smx[0][target_label]
    print smx[0]
    print "initial GT confidence: ", gtcnf
    print "initial TARGET confidence",targetcnf
    ...
    garbage_pred,_,garbage_smax = classify_stack(test_model, garbage_stack)
    ...
    for i in range(MAX_PERTS):
        ...
        _,_,prev_iter_smax = classify_stack(test_model, check_prev_iter_stack)
        prev_iter_conf = prev_iter_smax[0][target_label]
        ...
        ### Option 2: Manually select start loc
        ...
        init_pred,_,init_smax = classify_stack(test_model, tmp_oflow_stack)
        ...
        print init_smax[0]
        init_conf = init_smax[0][target_label]
        print "TARGET confidence: ", init_conf
        ...
        for img_arr_index in range(1,len(tmp_img_arr)):
            ...
            curr_pred,_,s = classify_stack(test_model, tmp_of)
            init_conf = s[0][target_label]
            ...
            print "new TARGET confidence:", init_conf
        ...
        # Find the index of the highest conficence score for the run
        best_ind,highest_pert_conf = max(enumerate(saved_confidence_scores), key=operator.itemgetter(1))
        tmp_img_arr = np.copy(saved_image_arrays[best_ind])

        print "\n\n"
        print "End of perturbation sequence: {}! Best Achieved Confidence: {}".format(i,highest_pert_conf)
        print "\n"
